...
export default function page({ params }: { params: { dmId: string } }) {
  ...
  const checkLimitCharacters = (input: string) => {
    if (input.length > 512)
      return toast.error("you can't send more than 512 characters");
    setInputValue(input);
  };
  ...
  return (
    <>
      ...
      <LefttsideModaL>
        ...
        <section>
          {ConversationList && ConversationList.length ? (
            ConversationList.map((md: any, key: number) => (
      ...
      <div className=" relative flex flex-col items-center w-full">
        {ConversationInfo ? (
          <div
            className={`Conversations relative w-full  h-[83vh] md:h-[88vh] flex flex-col sm:flex`}
          >
            <section className="bg-[#243230] py-3 px-8 flex gap-8 items-center">
              <Image
                onClick={handlePublicProfile}
                src={scUser ? scUser.avatar : ''}
                alt="User avatar"
                width={55}
                height={55}
              ...
              <section className="flex items-center justify-between w-full text-[#1EF0AE]">
                ...
                <p className="text-primary">
                  {formatDate(ConversationInfo.updated_at.toString())}
                </p>
              </section>
            </section>
            {
              <div className="flex flex-col justify-between  h-[78vh] md:h-[83vh] pb-5 ">
                <div
                  ...
                  className="ConversationsMessages relative p-4 overflow-y-scroll flex flex-col gap-3"
                >
                  {!LoadingMessages ? (
                ...
                </div>
                {
                  <div className="w-full relative px-6">
                    {SendingMessage && (
                      <div className=" text-secondary text-[10px] capitalize absolute -top-4 left-4 bg-[#161F1E] ">
                        sending ....
                      </div>
                    )}
                    <div className="ConversationsInput w-full h-[54px] bg-[#24323044] text-[#ffffff]  text-[16px]  rounded-[12px] flex justify-end items-center">
                      ...
                      {InputValue.length !== 0 && (
                        <div className="flex gap-1">
                          <span className=" text-[8px] text-center max-w-max flex w-full">
